...
class ClusterNetworkModel:
    ...
    def __init__(
        ...
        verbose = True,
        datatype = tensorflow.float64) -> None:
        """
    ...
    def CreateModel(
        self,
        input_NN,input_size,output_size,
        mins_input,maxs_input,
        mins_output,maxs_output
    ):

        self.PrintIf("> Creating model...")

        # Normalize input
        input = 2.0 * (input_NN - mins_input) / (maxs_input-mins_input) - 1.0
        ...
        with tensorflow.name_scope("ContextNN"):
        ...
        if self.number_of_pairs_FuncContext > 0:
            with tensorflow.name_scope("out"):
                output_model = tensorflow.transpose(tensorflow.squeeze(
           ...
           output_model = output_FullyConnected

        # 'Denormalize' output
        self.model = (output_model + 1.0) * (maxs_output-mins_output) / 2.0 + mins_output
